/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package v1

import (
	"context"
	"time"

	v1 "github.com/alibaba/kube-sharding/pkg/apis/carbon/v1"
	scheme "github.com/alibaba/kube-sharding/pkg/client/clientset/versioned/scheme"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	rest "k8s.io/client-go/rest"
)

// WorkerNodeEvictionsGetter has a method to return a WorkerNodeEvictionInterface.
// A group's client should implement this interface.
type WorkerNodeEvictionsGetter interface {
	WorkerNodeEvictions(namespace string) WorkerNodeEvictionInterface
}

// WorkerNodeEvictionInterface has methods to work with WorkerNodeEviction resources.
type WorkerNodeEvictionInterface interface {
	Create(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.CreateOptions) (*v1.WorkerNodeEviction, error)
	Update(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.UpdateOptions) (*v1.WorkerNodeEviction, error)
	UpdateStatus(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.UpdateOptions) (*v1.WorkerNodeEviction, error)
	Delete(ctx context.Context, name string, opts metav1.DeleteOptions) error
	DeleteCollection(ctx context.Context, opts metav1.DeleteOptions, listOpts metav1.ListOptions) error
	Get(ctx context.Context, name string, opts metav1.GetOptions) (*v1.WorkerNodeEviction, error)
	List(ctx context.Context, opts metav1.ListOptions) (*v1.WorkerNodeEvictionList, error)
	Watch(ctx context.Context, opts metav1.ListOptions) (watch.Interface, error)
	Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts metav1.PatchOptions, subresources ...string) (result *v1.WorkerNodeEviction, err error)
	WorkerNodeEvictionExpansion
}

// workerNodeEvictions implements WorkerNodeEvictionInterface
type workerNodeEvictions struct {
	client rest.Interface
	ns     string
}

// newWorkerNodeEvictions returns a WorkerNodeEvictions
func newWorkerNodeEvictions(c *CarbonV1Client, namespace string) *workerNodeEvictions {
	return &workerNodeEvictions{
		client: c.RESTClient(),
		ns:     namespace,
	}
}

// Get takes name of the workerNodeEviction, and returns the corresponding workerNodeEviction object, and an error if there is any.
func (c *workerNodeEvictions) Get(ctx context.Context, name string, options metav1.GetOptions) (result *v1.WorkerNodeEviction, err error) {
	result = &v1.WorkerNodeEviction{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("workernodeevictions").
		Name(name).
		VersionedParams(&options, scheme.ParameterCodec).
		Do(ctx).
		Into(result)
	return
}

// List takes label and field selectors, and returns the list of WorkerNodeEvictions that match those selectors.
func (c *workerNodeEvictions) List(ctx context.Context, opts metav1.ListOptions) (result *v1.WorkerNodeEvictionList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &v1.WorkerNodeEvictionList{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("workernodeevictions").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Do(ctx).
		Into(result)
	return
}

// Watch returns a watch.Interface that watches the requested workerNodeEvictions.
func (c *workerNodeEvictions) Watch(ctx context.Context, opts metav1.ListOptions) (watch.Interface, error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	opts.Watch = true
	return c.client.Get().
		Namespace(c.ns).
		Resource("workernodeevictions").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Watch(ctx)
}

// Create takes the representation of a workerNodeEviction and creates it.  Returns the server's representation of the workerNodeEviction, and an error, if there is any.
func (c *workerNodeEvictions) Create(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.CreateOptions) (result *v1.WorkerNodeEviction, err error) {
	result = &v1.WorkerNodeEviction{}
	err = c.client.Post().
		Namespace(c.ns).
		Resource("workernodeevictions").
		VersionedParams(&opts, scheme.ParameterCodec).
		Body(workerNodeEviction).
		Do(ctx).
		Into(result)
	return
}

// Update takes the representation of a workerNodeEviction and updates it. Returns the server's representation of the workerNodeEviction, and an error, if there is any.
func (c *workerNodeEvictions) Update(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.UpdateOptions) (result *v1.WorkerNodeEviction, err error) {
	result = &v1.WorkerNodeEviction{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("workernodeevictions").
		Name(workerNodeEviction.Name).
		VersionedParams(&opts, scheme.ParameterCodec).
		Body(workerNodeEviction).
		Do(ctx).
		Into(result)
	return
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *workerNodeEvictions) UpdateStatus(ctx context.Context, workerNodeEviction *v1.WorkerNodeEviction, opts metav1.UpdateOptions) (result *v1.WorkerNodeEviction, err error) {
	result = &v1.WorkerNodeEviction{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("workernodeevictions").
		Name(workerNodeEviction.Name).
		SubResource("status").
		VersionedParams(&opts, scheme.ParameterCodec).
		Body(workerNodeEviction).
		Do(ctx).
		Into(result)
	return
}

// Delete takes name of the workerNodeEviction and deletes it. Returns an error if one occurs.
func (c *workerNodeEvictions) Delete(ctx context.Context, name string, opts metav1.DeleteOptions) error {
	return c.client.Delete().
		Namespace(c.ns).
		Resource("workernodeevictions").
		Name(name).
		Body(&opts).
		Do(ctx).
		Error()
}

// DeleteCollection deletes a collection of objects.
func (c *workerNodeEvictions) DeleteCollection(ctx context.Context, opts metav1.DeleteOptions, listOpts metav1.ListOptions) error {
	var timeout time.Duration
	if listOpts.TimeoutSeconds != nil {
		timeout = time.Duration(*listOpts.TimeoutSeconds) * time.Second
	}
	return c.client.Delete().
		Namespace(c.ns).
		Resource("workernodeevictions").
		VersionedParams(&listOpts, scheme.ParameterCodec).
		Timeout(timeout).
		Body(&opts).
		Do(ctx).
		Error()
}

// Patch applies the patch and returns the patched workerNodeEviction.
func (c *workerNodeEvictions) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts metav1.PatchOptions, subresources ...string) (result *v1.WorkerNodeEviction, err error) {
	result = &v1.WorkerNodeEviction{}
	err = c.client.Patch(pt).
		Namespace(c.ns).
		Resource("workernodeevictions").
		Name(name).
		SubResource(subresources...).
		VersionedParams(&opts, scheme.ParameterCodec).
		Body(data).
		Do(ctx).
		Into(result)
	return
}
